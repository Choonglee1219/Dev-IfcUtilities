...
*   **Request Body**:
    ...
    *   `rotationAngle`: 회전각 (도(degree) 단위, `float`, 선택사항. 내부적으로 2D 회전 방향 벡터 `xAxisAbscissa`, `xAxisOrdinate`로 자동 변환)
    *   `xAxisAbscissa`: X축 방향 코사인 값 ($\cos\theta$, `float`, 선택사항)
    *   `xAxisOrdinate`: X축 방향 사인 값 ($\sin\theta$, `float`, 선택사항)
